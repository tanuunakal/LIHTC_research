...
class SetupFunctions:
  ...
  def construct_drive_url (self, file_id):
  ...
  def get_boundaries(self, year, section_name):
  ...
  def get_year_by_name(self, folder_name):
  ...
  def standardize_dict_project_location(self, user_selections, data):
  ...
  def get_data_by_drive_id(self, drive_id, sheet_name, year,
    ...
    wb = self.load_workbook_from_url (self.construct_drive_url (drive_id))
    ...
    data = self.get_data_for_sheet (wb, sheet_name, user_selections, boundaries[0],
                              boundaries[1])
    if section_name == "Applicant Contact":
      self.standardize_dict_applicant_contact(user_selections, data)
    if section_name == "Project Location":
      self.standardize_dict_project_location(user_selections, data)
    return data
  ...
  def get_data_for_year(self, drive_dir_id, sheet_name,
                        year, section_name):
    data = []
    for file_info in tqdm(self.folder_list (drive_dir_id)):
      f_id = file_info[0]
      f_name = file_info[1]
      print("f name is: " + f_name)
      boundaries = self.get_boundaries(year, section_name)
      if f_id in weird_ids and section_name == "Project Location":
        boundaries = [25, 44]
      data_file = self.get_data_by_drive_id(file_info[0], sheet_name,
  ...
  def write_all_files_to_json(self, drive_dir_id, section_name):
    data = []
    for file_info in tqdm (self.folder_list (drive_dir_id)):
      year_str = self.get_year_by_name(file_info[1])
      print(year_str)
      yearly_data = self.get_data_for_year(file_info[0], self.get_sheet_name(year_str),
